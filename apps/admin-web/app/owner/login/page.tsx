...
export default function OwnerLoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setError(null)
    const result = await ownerApi<{ owner: Owner }>('/auth/owner/login', {
      method: 'POST',
      body: data,
    })
    if (!result.ok) {
  ...
  const handleGoogleLogin = () => {
    ...
    window.location.href = `${base}/api/v1/auth/owner/google`
  }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">Sign in to manage your venues</p>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-card">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between">
                ...
                <Link href="/owner/forgot-password" className="text-xs text-emerald-600 hover:underline">
                  Forgot password?
                </Link>
              </div>
          ...
          <button
            ...
            className="flex h-10 w-full items-center justify-center gap-3 rounded-md border border-gray-200 bg-white text-sm font-medium text-gray-700 hover:bg-gray-50"
          >
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            ...
            <Link href="/owner/register" className="font-medium text-emerald-600 hover:underline">
              Sign up
            </Link>
          </p>
        </div>
