...
import { UserRow, UserRowData } from '@/components/users/UserRow';
...
export default function UsersScreen() {
  ...
  const fetchUsers = useCallback(async () => {
    setError(null);
    const result = await superAdminApi<UsersResponse>('/api/v1/super-admin/users');
    if (result.ok) {
  ...
  const filtered = users.filter((u) => {
    ...
    const matchesFilter =
      activeFilter === 'all' || (activeFilter === 'blocked' && u.status === 'blocked');
